...
func TestProtectorRequire(t *testing.T) {
	...
	limits := RateLimits{AnonRPM: 60, ConsumerRPM: 120, SubscriberRPM: 120, AdminRPM: 300, AdminIngestRPM: 10}
	...
	tests := []struct {
		...
		},
		{
			name:       "missing key",
			allowed:    ExternalRoles,
			consume:    store.RateLimitResult{Allowed: true, Limit: 60, Remaining: 59, ResetUnix: 1},
